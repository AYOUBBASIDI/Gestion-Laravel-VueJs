...
export default {
  ...
  return {
    ...
    mounted(){
        ...
        axios(options)
        .then(response => {
            this.stats[2].count = response.data.length;
        });
        ...
        axios(options)
        .then(response => {
            this.stats[1].count = response.data.length;
        });
        if(sessionStorage.getItem('role') == "Admin"){
            ...
            axios(options)
            .then(response => {
                this.stats[0].count = response.data.length;
                this.stats[0].title = 'Total Users'
            });
        }else{
            // Get Proprietaire
            options = {
            ...
            .then(response => {
                this.stats[0].count = response.data.length;
                this.stats[0].title = 'Total Proprietaires'
            });
        }
