<template>
    <div class="p-4 space-y-4">
        <PageHeader title="Agents" subtitle="Listing all Records">
            <div class="mt-1">
                <input
                    v-bind="$attrs"
            ...
            <BaseButton @click="openPopUpAdd()">Add New User</BaseButton>
        ...
        <div class="border-b border-gray-200 dark:border-dark-body">
            <table class="relative rounded-table w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody class="bg-white dark:bg-dark-header divide-y divide-gray-200 dark:divide-gray-700">
                    <tr v-for="item in dataStore" class="">
                        ...
                        <td class="table-data" v-html="highlightMatches(item.username)">
                        </td>
                        <td class="table-data">
                            {{item.email}}
                        </td>
                        ...
                        <td class="table-data">
                            {{item.parcelles.length}}
                        </td>
                        <td class="table-data">
                            <BaseButton design="info" @click="openPopUpUpdate(item.id)"><i class="fa-solid fa-pen-to-square"></i></BaseButton> | <BaseButton design="danger" v-on:click="DeleteAgent(item.id)"><i class="fa-solid fa-trash-can"></i></BaseButton>
                        </td>
                    </tr>
                </tbody>
            ...
            <AddAgent v-if="popupAdd"></AddAgent>
            <UpdateAgent v-if="popupUpdate" :id="userId"></UpdateAgent>
    ...
    methods:{
        ...
        Search(){
            ...
            this.dataStore = this.data.filter(Proprietaire => {
              const name = Proprietaire.username.toString().toLowerCase();
