    ...
    <!-- <form v-on:submit.prevent="update_Parcelle(id)">
            ...
            <div>
               <BaseLabel>Nom de village</BaseLabel>
                <div class="mt-1">
                    ...
                    >
                        <option v-for="item in villages" :value='item.id' :selected="selectedVillage == item.id ? true : false">{{item.nom}}</option>
                    </select>
                </div>
                <FormError :error="error" /> 
            </div>
        ...
        <div>
            ...
            <div class="mt-1">
                ...
                >
                    <option v-for="item in proprietaires" :value="item.id" :selected="selectedProp == item.id ? true : false">{{item.numero_identite}}</option>
                </select>
            </div>
            <FormError :error="error" />
        </div> 
        ...
        <div :class="{ hidden: !role }">
            ...
            <div class="mt-1">
                ...
                class="focus:ring-0 focus:border-primary dark:focus:border-gray-200 bg-inherit block w-full sm:text-sm border-0 border-b-2 border-gray-300 dark:border-gray-700 dark:text-gray-200"
                name="user_id">
                    <option value="hello" selected>zadazd</option>
...
update_Parcelle(id){
    ...
    getData(id){
        const options = {
            url: 'http://127.0.0.1:8000/api/v1/getParcelle/'+id,
            method: 'GET',
            headers: {
                'Accept': 'application/json',
                'Content-Type': 'application/json;charset=UTF-8'
        ...
        .then(response => {
            ...
            this.form.date_delimitation = response.data.date_delimitation;
            this.form.village_id = response.data.village_id;
            this.form.proprietaire_id = response.data.proprietaire_id;
